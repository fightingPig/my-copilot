...
OUTPUT_FILE="$PROJECT_DIR/.claude/java-coding-standards.md"

PROJECT_CONFIG_PATH="$CONFIG_FILE" OUTPUT_FILE="$OUTPUT_FILE" python3 - <<'PY'
import json
import os

config_path = os.environ["PROJECT_CONFIG_PATH"]
...
- JDK：{project.get('jdkVersion', '8')}
- {project.get('framework', 'Spring Boot')}：{project.get('frameworkVersion', '3.2.x')}
- 持久层：{project.get('persistenceFramework', 'MyBatis-Plus 3.5.x')}
- 架构：{project.get('architecture', 'layered')}
- 日志框架：SLF4J
...
echo "已根据 project-config.json 生成 $OUTPUT_FILE"
